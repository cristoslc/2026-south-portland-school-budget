...
Deliverables are published to `dist/transportation-analysis/`:

```
dist/transportation-analysis/
  README.md                               — Overview, key findings, contents
  METHODOLOGY.md                          — All assumptions, data sources, what would improve the analysis
  briefings/                              — 15 persona-specific + 1 general transport briefs
  transport-configuration-comparison.md   — Capstone comparison document
  split-family-model.md                   — SPEC-060 analysis
  mckinney-vento-exposure.md              — SPEC-061 analysis
  sea-staffing-assessment.md              — SPEC-062 analysis
  bell-schedule-analysis.md               — SPEC-063 analysis
  before-after-care-gap.md                — SPEC-064 analysis
  transport-comparison.json               — Machine-readable data for site consumption
```

Analysis source documents also live in `docs/analysis/`. Calculation scripts are in `pipeline/transport/`. Raw model outputs are in `data/` as JSON files. The site ([INITIATIVE-004](../(INITIATIVE-004)-Public-Budget-Site/(INITIATIVE-004)-Public-Budget-Site.md)) renders these as web pages via Astro content collections.

Transport persona briefs are generated by `scripts/generate_transport_briefs.py` using the shared LLM client (`pipeline/llm_client.py`).
