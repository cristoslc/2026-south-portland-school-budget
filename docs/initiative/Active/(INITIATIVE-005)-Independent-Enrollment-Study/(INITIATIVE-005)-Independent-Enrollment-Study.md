...
Deliverables are published to `dist/enrollment-study/`:

```
dist/enrollment-study/
  README.md               — Overview, key findings, contents
  METHODOLOGY.md          — Assumptions, data sources, reproducibility
  briefings/              — 15 persona-specific + 2 general briefings
  nces-enrollment-by-grade.csv
  nces-enrollment-by-grade.json
```

Briefings are generated by the interpretation pipeline (`scripts/generate_briefs.py` → `pipeline/llm_client.py`) and published via `scripts/poll_interpret.py`. The site ([INITIATIVE-004](../Active/(INITIATIVE-004)-Public-Budget-Site/(INITIATIVE-004)-Public-Budget-Site.md)) renders these as web pages via Astro content collections.

Source data lives in `data/enrollment/`. Analysis artifacts live in `docs/analysis/`.
